...
setwd("/work/LAS/jfw-lab/del_mut_gvcf/BQ20_gVCFs/gvcfs/frqs")
...
REF <- REF[,c(1,2,3,seq(6,38,2))]

ALT <- in_frqs[in_frqs$ALT == 1,]
ALT <- ALT[,c(1,2,3,seq(7,39,2))]

names(REF) <- c("CHROM", "POS", "CHROM_POS", "Outgroup", "A1D", "A1W", "A2W", "AD1D", "AD1L1", "AD1L2", 
                "AD1W", "AD2D", "AD2L1", "AD2L2", "AD3", "AD4", "AD5", "AD6", 
                "AD7", "D5")
names(ALT) <- c("CHROM", "POS", "CHROM_POS", "Outgroup", "A1D", "A1W", "A2W", "AD1D", "AD1L1", "AD1L2", 
                "AD1W", "AD2D", "AD2L1", "AD2L2", "AD3", "AD4", "AD5", "AD6", 
                "AD7", "D5")
...
final_vcf <- merge(x=GERP_vcf, y=BAD_results, by.x = c("CHROM_POS"), by.y = c("SNP.Name"), all.x = TRUE)


write.table(final_vcf, file = "all.frqs.BAD.allGERP.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

GERP06 <- subset(final_vcf, GERP > 0 | !is.na(GeneID))
write.table(GERP06, file = "GERP06_BAD.frqs.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

GERP_melt <- melt(GERP06, measure.vars = c("A1D", "A1W", "A2W", "AD1D", "AD1L1", "AD1L2",
                "AD1W", "AD2D", "AD2L1", "AD2L2", "AD3", "AD4", "AD5", "AD6",
