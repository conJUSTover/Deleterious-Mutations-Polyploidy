...
chr01_vcf_in <- read.table(gzfile('05.biallele.recode.nocolon.vcf.gz'), header = FALSE)
names(chr01_vcf_in) <- vcf_header

#add ancestral state to new column 
Gkirkii_seq <- read.table('roast.05.msa.Gkirkii.seqlines', header = FALSE)
...
gerp_score <- read.table('roast.05.msa.in.gaps.rates', header = FALSE)
names(gerp_score) <- c('Neutral Rate','RS Score')
chr01_vcf_filter1$GERPgap0 <- gerp_score[chr01_vcf_filter1$POS,2]

gerp_score <- read.table('roast.05.msa.in.new.rates', header = FALSE)
names(gerp_score) <- c('Neutral Rate','RS Score')
...
write.table(PostCounts, "05.DerivedCount.GERP.txt", sep='\t', row.names=F, col.names = F, quote=F)
