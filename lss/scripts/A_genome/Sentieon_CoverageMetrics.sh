...
#time ( bwa mem -M -K 10000000 -R "@RG\tID:$1 \tSM:$1 \tPL:ILLUMINA" -t $2 $3 $1.1.fq.gz $1.2.fq.gz > $1.sam  ) &> $1.bwa.timelog

#samtools view -F 260 -bu -@ $2 $1.sam | samtools sort -@ $2 -m 4G -o $1.sort.bam && rm $1.sam
...
time( sentieon driver -t $2 -r $3 -i $1.realign.bam --algo CoverageMetrics --omit_base_output $1.Coverage.metric) &> $1.CoverageMetrics.timelog
