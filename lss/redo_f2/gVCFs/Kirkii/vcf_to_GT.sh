...
# Find Sites that are present in outgroup and not segregating
vcftools --vcf $1.biallele.recode.vcf --keep out.lst --freq2 --out $1.outgroup
grep -v "CHROM" $1.outgroup.frq | awk 'OFS="\t" {if($6 == 1) {print $1,$2}}' > $1.keep.pos
grep -v "CHROM" $1.outgroup.frq | awk 'OFS="\t" {if($6 == 0) {print $1,$2}}' >> $1.keep.pos
