...
in_frqs <- read.table("out.pSONIC.not_quartet.frq", skip = 1)
...
file.names <- list.files(pattern = "*.pSONIC.not_quartet.frq$", recursive = F)
file.names <- file.names[-length(file.names)]

for(i in 1:length(file.names)){
  current.file <- file.names[i]
  name.base <- sub(".pSONIC.not_quartet.frq", "", current.file)
  print(name.base)
...
write.table(final_vcf, file = "all.frqs.BAD.allGERP.pSONIC.not_quartet.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

GERP06 <- subset(final_vcf, (GERP > -2 & GERP != 0) | !is.na(GeneID) | !is.na(Synonymous))
write.table(GERP06, file = "GERP06_BAD.frqs.pSONIC.not_quartet.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
...
write.table(GERP_melt, file="GERP06.melted.frqs.pSONIC.not_quartet.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

GERP_melt <- GERP_melt[c(GERP_melt$DAF > 0 & !is.na(GERP_melt$DAF)),]

write.table(GERP_melt, file="GERP06.melted.frqs.seg.pSONIC.not_quartet.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
