...
write.table(GERP_melt, file="GERP06.melted.frqs.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)

GERP_melt <- GERP_melt[c(GERP_melt$DAF > 0 & !is.na(GERP_melt$DAF)),]

write.table(GERP_melt, file="GERP06.melted.frqs.seg.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
