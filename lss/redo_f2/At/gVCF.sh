...
#samtools view -F 260 -bu -@ $2 $1.sam | samtools view -f 2 -bu -@ $2 | samtools sort -m 4G -@ $2 -o $1.sort.bam && rm $1.sam
...
time ( sentieon driver -t $2 -r $3 -i $1.realign.bam --algo Haplotyper $1.gVCF --min_base_qual 20 --emit_mode gvcf ) &> $1.gvcf.timelog
