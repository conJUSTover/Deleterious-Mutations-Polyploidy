...
infile <- read.table("GERP06_BAD.frqs.pSONIC.NO_GC.txt", header = TRUE)
infile <- infile[complete.cases(infile[,5:12]),]
...
Synonymous_genespace_At <- function(species){
  temp <- infile[c(!is.na(infile[,species]) & infile$CHROM < 14 & infile$Synonymous == "synonymous_variant" & !is.na(infile$Synonymous)),]
  return(sum(temp[,species]))
}

Synonymous_genespace_Dt <- function(species){
  temp <- infile[c(!is.na(infile[,species]) & infile$CHROM > 13 & infile$Synonymous == "synonymous_variant" & !is.na(infile$Synonymous)),]
  return(sum(temp[,species]))
}

Missense_genespace_At <- function(species){
  temp <- infile[c(!is.na(infile[,species]) & infile$CHROM < 14 & infile$Synonymous == "missense_variant" & !is.na(infile$Synonymous)),]
  return(sum(temp[,species]))
}

Missense_genespace_Dt <- function(species){
  temp <- infile[c(!is.na(infile[,species]) & infile$CHROM > 13 & infile$Synonymous == "missense_variant" & !is.na(infile$Synonymous)),]
  return(sum(temp[,species]))
}
...
Synon_bygene_all <- function(gene, Species){
  temp <- infile[c(infile$Syn_Gene == gene & infile$Synonymous == "synonymous_variant"),]
  output <- sapply(Species, BAD_bygene, df=temp)
  return(output)
}

Nonsynon_bygene_all <- function(gene, Species){
  temp <- infile[c(infile$Syn_Gene == gene & infile$Synonymous == "missense_variant"),]
  output <- sapply(Species, BAD_bygene, df=temp)
  return(output)
}
...
GERP_loads$At_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_At))
GERP_loads$Dt_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_Dt))
GERP_loads$At_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_At))
GERP_loads$Dt_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_Dt))
...
###Synonymous between homoeologs 
homoeologs[, c("At_AD1_Synon_load", "At_AD3_Synon_load", "At_AD4_Synon_load", "At_AD5_Synon_load", "At_AD6_Synon_load", "At_AD7_Synon_load", "At_A1_Synon_load", "At_D5_Synon_load")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Synon_load", "Dt_AD3_Synon_load", "Dt_AD4_Synon_load", "Dt_AD5_Synon_load", "Dt_AD6_Synon_load", "Dt_AD7_Synon_load", "Dt_A1_Synon_load", "Dt_D5_Synon_load")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))

print("Synonymous done")

###Nonsynonymous between homoeologs 
homoeologs[, c("At_AD1_Nonsynon_load", "At_AD3_Nonsynon_load", "At_AD4_Nonsynon_load", "At_AD5_Nonsynon_load", "At_AD6_Nonsynon_load", "At_AD7_Nonsynon_load", "At_A1_Nonsynon_load", "At_D5_Nonsynon_load")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Nonsynon_load", "Dt_AD3_Nonsynon_load", "Dt_AD4_Nonsynon_load", "Dt_AD5_Nonsynon_load", "Dt_AD6_Nonsynon_load", "Dt_AD7_Nonsynon_load", "Dt_A1_Nonsynon_load", "Dt_D5_Nonsynon_load")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))

print("Nonsynonymous done")
...
GERP_loads$At_diploid_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_At))
GERP_loads$Dt_diploid_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_Dt))
GERP_loads$At_diploid_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_At))
GERP_loads$Dt_diploid_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_Dt))
...
###Synonymous between homoeologs 
homoeologs[, c("At_AD1_Synon_load_dip", "At_AD3_Synon_load_dip", "At_AD4_Synon_load_dip", "At_AD5_Synon_load_dip", "At_AD6_Synon_load_dip", "At_AD7_Synon_load_dip", "At_A1_Synon_load_dip", "At_D5_Synon_load_dip")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Synon_load_dip", "Dt_AD3_Synon_load_dip", "Dt_AD4_Synon_load_dip", "Dt_AD5_Synon_load_dip", "Dt_AD6_Synon_load_dip", "Dt_AD7_Synon_load_dip", "Dt_A1_Synon_load_dip", "Dt_D5_Synon_load_dip")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))

print("Synonymous done")

###Nonsynonymous between homoeologs 
homoeologs[, c("At_AD1_Nonsynon_load_dip", "At_AD3_Nonsynon_load_dip", "At_AD4_Nonsynon_load_dip", "At_AD5_Nonsynon_load_dip", "At_AD6_Nonsynon_load_dip", "At_AD7_Nonsynon_load_dip", "At_A1_Nonsynon_load_dip", "At_D5_Nonsynon_load_dip")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Nonsynon_load_dip", "Dt_AD3_Nonsynon_load_dip", "Dt_AD4_Nonsynon_load_dip", "Dt_AD5_Nonsynon_load_dip", "Dt_AD6_Nonsynon_load_dip", "Dt_AD7_Nonsynon_load_dip", "Dt_A1_Nonsynon_load_dip", "Dt_D5_Nonsynon_load_dip")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
...
GERP_loads$At_polyploid_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_At))
GERP_loads$Dt_polyploid_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_Dt))
GERP_loads$At_polyploid_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_At))
GERP_loads$Dt_polyploid_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_Dt))
...
###Synonymous between homoeologs 
homoeologs[, c("At_AD1_Synon_load_poly", "At_AD3_Synon_load_poly", "At_AD4_Synon_load_poly", "At_AD5_Synon_load_poly", "At_AD6_Synon_load_poly", "At_AD7_Synon_load_poly", "At_A1_Synon_load_poly", "At_D5_Synon_load_poly")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Synon_load_poly", "Dt_AD3_Synon_load_poly", "Dt_AD4_Synon_load_poly", "Dt_AD5_Synon_load_poly", "Dt_AD6_Synon_load_poly", "Dt_AD7_Synon_load_poly", "Dt_A1_Synon_load_poly", "Dt_D5_Synon_load_poly")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))

print("Synonymous done")

###Nonsynonymous between homoeologs 
homoeologs[, c("At_AD1_Nonsynon_load_poly", "At_AD3_Nonsynon_load_poly", "At_AD4_Nonsynon_load_poly", "At_AD5_Nonsynon_load_poly", "At_AD6_Nonsynon_load_poly", "At_AD7_Nonsynon_load_poly", "At_A1_Nonsynon_load_poly", "At_D5_Nonsynon_load_poly")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Nonsynon_load_poly", "Dt_AD3_Nonsynon_load_poly", "Dt_AD4_Nonsynon_load_poly", "Dt_AD5_Nonsynon_load_poly", "Dt_AD6_Nonsynon_load_poly", "Dt_AD7_Nonsynon_load_poly", "Dt_A1_Nonsynon_load_poly", "Dt_D5_Nonsynon_load_poly")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
...
GERP_loads$Dt_seg_model_genespace_BAD <- unlist(lapply(GERP_loads$Species, BAD_load_genespace_Dt))
GERP_loads$At_seg_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_At))
GERP_loads$Dt_seg_model_genespace_synonymous <- unlist(lapply(GERP_loads$Species, Synonymous_genespace_Dt))
GERP_loads$At_seg_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_At))
GERP_loads$Dt_seg_model_genespace_missense <- unlist(lapply(GERP_loads$Species, Missense_genespace_Dt))
...
###Synonymous between homoeologs 
homoeologs[, c("At_AD1_Synon_load_seg", "At_AD3_Synon_load_seg", "At_AD4_Synon_load_seg", "At_AD5_Synon_load_seg", "At_AD6_Synon_load_seg", "At_AD7_Synon_load_seg", "At_A1_Synon_load_seg", "At_D5_Synon_load_seg")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Synon_load_seg", "Dt_AD3_Synon_load_seg", "Dt_AD4_Synon_load_seg", "Dt_AD5_Synon_load_seg", "Dt_AD6_Synon_load_seg", "Dt_AD7_Synon_load_seg", "Dt_A1_Synon_load_seg", "Dt_D5_Synon_load_seg")] <- t(apply(homoeologs, 1, function(x) Synon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))

print("Synonymous done")

###Nonsynonymous between homoeologs 
homoeologs[, c("At_AD1_Nonsynon_load_seg", "At_AD3_Nonsynon_load_seg", "At_AD4_Nonsynon_load_seg", "At_AD5_Nonsynon_load_seg", "At_AD6_Nonsynon_load_seg", "At_AD7_Nonsynon_load_seg", "At_A1_Nonsynon_load_seg", "At_D5_Nonsynon_load_seg")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[1], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
homoeologs[, c("Dt_AD1_Nonsynon_load_seg", "Dt_AD3_Nonsynon_load_seg", "Dt_AD4_Nonsynon_load_seg", "Dt_AD5_Nonsynon_load_seg", "Dt_AD6_Nonsynon_load_seg", "Dt_AD7_Nonsynon_load_seg", "Dt_A1_Nonsynon_load_seg", "Dt_D5_Nonsynon_load_seg")] <- t(apply(homoeologs, 1, function(x) Nonsynon_bygene_all(x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))))
...
write.table(GERP_loads, "Loads_Total_GERP_Bad.NO_GC.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
write.table(homoeologs, "Homoeologs_Load_per_Pop.NO_GC.txt", quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
