...
At$Species <- factor(At$Species, levels = c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"))
Dt <- new_sums[new_sums$Subgenome == "Dt",]
Dt$Species <- factor(Dt$Species, levels = c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "D5", "A1"))
Dtold <- Dt[Dt$Species != "A1",]
Dtnew <- Dt[Dt$Species == "A1",]
...
ggsave("Figure_GERP_Proportions_all.pdf", width = 11, height = 6, units = "cm")
