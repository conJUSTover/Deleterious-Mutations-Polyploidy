...
new_sums <- new_sums[!c(new_sums$Species == "D5" & new_sums$Subgenome == "At"),]
new_sums <- new_sums[!c(new_sums$Species == "A1" & new_sums$Subgenome == "Dt"),]
...
ggsave("Figure_GERP_Proportions.pdf", width = 11, height = 6, units = "cm")
...
#Constraint 
BAD_bygene_all <- function(low, high_inc, Species, chr_low, chr_high){
  temp <- temp_constraint[c(temp_constraint$MaskedConstraint < high_inc & temp_constraint$MaskedConstraint > low & !is.na(temp_constraint$MaskedConstraint) & temp_constraint$CHROM >= chr_low & temp_constraint$CHROM <= chr_high),]
  output <- sapply(Species, BAD_bypos, df=temp)
  return(output)
}

temp_constraint <- infile[c(infile$MaskedConstraint > 0 & !is.na(infile$MaskedConstraint)),]

BAD_cumulative <- data.frame(Low = c(0, 0.001, 0.01, 0.1, 1), High = c(0.001, 0.01, 0.1, 1, 10000))

BAD_cumulative[, c("At_AD1", "At_AD3", "At_AD4", "At_AD5", "At_AD6", "At_AD7", "At_A1", "At_D5")] <- t(apply(BAD_cumulative, 1, function(x) BAD_bygene_all(x[1], x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"), chr_low = 1, chr_high = 13)))
BAD_cumulative[, c("Dt_AD1", "Dt_AD3", "Dt_AD4", "Dt_AD5", "Dt_AD6", "Dt_AD7", "Dt_A1", "Dt_D5")] <- t(apply(BAD_cumulative, 1, function(x) BAD_bygene_all(x[1], x[2], c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "A1", "D5"), chr_low = 14, chr_high = 26)))

sums <- melt(BAD_cumulative, id.vars = c("Low", "High"))
sums <- cbind(sums, read.table(text = as.character(sums$variable), sep = "_"))
names(sums) <- c("Low", "High", "variable", "Load", "Subgenome", "Species")

sums <- sums[!c(sums$Species == "D5" & sums$Subgenome == "At"),]
sums <- sums[!c(sums$Species == "A1" & sums$Subgenome == "Dt"),]
sums$Species  = rep(c("AD1", "AD3", "AD4", "AD5", "AD6", "AD7", "Diploid"), each = 5)

At <- sums[sums$Subgenome == "At",]
Dt <- sums[sums$Subgenome == "Dt",]
...
sums <- rbind(At2, Dt2)
sums$Low <- as.factor(sums$Low)

sums$Constraint <- paste(sums$Low, sums$High, sep = " > dN/dS > ")

sums$Constraint <- as.factor(sums$Constraint)



Constraint_plot <- ggplot(sums, aes(x = Constraint, y = New, fill = Constraint)) + 
  geom_bar(stat = "identity") + theme_bw() + scale_fill_grey(start=0, end=0.85) + ggtitle("BAD_Mutations Relative Load") +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank(), legend.position = "bottom") +
  facet_grid(Subgenome~Species, scales = "free_y") + ylab("Proportion of Diploid SNPs")
Constraint_plot

ggsave("Figure_BAD_Mutations_Proportions.pdf", width = 22, height = 8, units = "cm")
