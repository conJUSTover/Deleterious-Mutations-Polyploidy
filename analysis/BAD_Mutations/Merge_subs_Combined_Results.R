...
subs <- read.table("all.GT.longform.subs")
names(subs) <- c("gene", "aapos", "altaa", "SNP Name")

##Create new variable for gene_aaPOS
BAD_table$GeneAA <- apply(BAD_table[,c("GeneID", "CDSPos")], 1, paste, collapse = "_")
subs$GeneAA <- apply(subs[,c("gene", "aapos")], 1, paste, collapse = "_")
